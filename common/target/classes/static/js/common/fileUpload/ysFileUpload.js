			...
			complete: function(response) {
				...
				if (spliterPos > 0) {
					...
					if(options.uploadType == "signupform"){
						...
						if (!options.noDelete) {
							fileDesc += "&nbsp;&nbsp;<a href='javascript:' class='txt-link' onclick='deleteFileName(this, \"oldFile\", \""+$('#'+uploadFormId+'>.uploadType').val()+"\", \""+uploadFormCounter+"\", \""+options.uploadFileFormParentId+"\");'><i class='fa fa-times'></i></a>";
						}
						fileDesc += "</div>";
						...
						if (!options.noDelete) {
							fileDesc += "&nbsp;&nbsp;<a href='javascript:' class='txt-link' onclick='deleteFileName(this, \"oldFile\", \""+$('#'+uploadFormId+'>.uploadType').val()+"\", \""+fileId+"\", \""+options.uploadFileFormParentId+"\");'><i class='fa fa-times'></i></a>";
						}
						fileDesc += "</div>";
						
						$(uploadFileFormParentIdStr + "#fu-message #new-file-flag").first().before(fileDesc);
						$("#attachmentFiles").val($("#attachmentFiles").val() + resText + '*');
					}
					$("#" + uploadFormId).remove();
					
					//uploadFormCounter -- ;
					var uploadMaxCount = $('form[id^="file-upload-comment-form"]').length;
					uploadFormCounter ++ ;
					if(uploadMaxCount > 0) {
...
function downloadFile(basePath, uploadType, attachmentId, uploadObjectId) {
	window.open(basePath+"/fs/download?ut="+uploadType+"&attachmentId="+attachmentId+"&uploadObjectId="+uploadObjectId, "_blank", "");
}
...
function addFile(uploadUrl, uploadType, uploadObjectId, maxUploadFileSize, uploadFileFormParentId) {
	if ((uploadType == "event" || uploadType == "eventplace" || uploadType == "eventinstructor" || uploadType == "eventcourse" || uploadType == "product" || uploadType == "membership" || uploadType == "survey") && attachmentNum >= 64) {
...
function showUploadFileName(file, maxUploadFileSize, uploadCounter, uploadType, uploadFileFormParentId) {
	...
	if(file.files[0].size > maxUploadFileSize){
		...
		else if((uploadType == "product" || uploadType == "membership" || uploadType == "eventplace" || uploadType == "eventinstructor" || uploadType == "event" || uploadType == "eventcourse" || uploadType == "survey") && attachmentNum >= 64) {
		    jzMsgBox('<m>追加できません。登録可能なファイルは64個までです</m>', 'alert');  
			return;
		}
		var filename = $(file).val().replace(/.*(\/|\\)/, "");
		var fileDesc = "<div style='max-width:490px; word-break:break-all;' id='new-upload-file" + uploadCounter + "'><i class='fa fa-paperclip'></i> <a href='javascript:;' class='txt-link' >" + filename + "</a> <a href='javascript:' class='txt-link' onclick='deleteFileName(this, \"newFile\", \"" + uploadType + "\", \"" + uploadCounter + "\", \"" + uploadFileFormParentId + "\")'><i class='fa fa-times'></i></a></div>";
		if (uploadType == 'originaldomain') {
		...
		if(uploadType == "mailmaga" || uploadType == "product" || uploadType == "membership" || uploadType == "eventplace" || uploadType == "eventinstructor" || uploadType == "event" || uploadType == "eventcourse" || uploadType == "fax" || uploadType == "survey") {
			attachmentNum ++;
		}
	}
	if (uploadType == "mailmaga" || uploadType == "event" || uploadType == "eventplace" || uploadType == "eventinstructor" || uploadType == "product" || uploadType == "eventcourse" || uploadType == "membership" || uploadType == "fax" || uploadType == "survey") {
		attachmentIconShow();
	}
}

function deleteFileName(file, type, uploadType, fileId, uploadFileFormParentId) {
	console.log("commonDeleteFileName");
	console.log(file);
	console.log(type);
	console.log(uploadType);
	console.log(fileId);
	console.log(uploadFileFormParentId);
	...
	if ($('#delete-the-attachment-link').length == 0) {
		if (uploadType != "contact") {
			$(file).parents('form:first').after('<a id="delete-the-attachment-link" href="#delete-the-attachment-div-link" style="display: none;"></a>' +
					'<div id="delete-the-attachment-div-link" class="tx-center ysdialog" >' +
					'<div class="ysdialog-contents">' +
					'「<span id="delete-attachment-file-name">' + attachmentFileName + '</span>」<m>を削除します</m>' +
					'</div>' +
					'<div class="ctrl-area">' +
					'<div class="ctrl-area__main">' +
					'<a class="ctrl-area__main__btn button ysdialog-close" href="javascript:;"><span><m>キャンセル</m></span></a>' +
					'<a id="delete-attachment-btn" onclick="deleteAttachmentfile();" href="javascript:;" class="ctrl-area__main__btn save-btn button ysdialog-close recommend link-trigger-excepted"><m>OK</m></a>' +
					'</div>' +
					'</div>' +
			'</div>');
		} else {
			$(file).parent().after('<a id="delete-the-attachment-link" href="#delete-the-attachment-div-link" style="display: none;"></a>' +
					...
					'<div class="ctrl-area__main">' +
					'<a class="ctrl-area__main__btn button ysdialog-close" href="javascript:;"><span><m>キャンセル</m></span></a>' +
					'<a id="delete-attachment-btn" onclick="deleteAttachmentfile();" href="javascript:;" class="ctrl-area__main__btn save-btn button ysdialog-close recommend"><m>OK</m></a>' +
					'</div>' +
					'</div>' +
			'</div>');
		}
	...
	$('#delete-the-attachment-link').YSModalDialog({dialogTitle: "<m>添付ファイルの削除</m>", dialogId: "deleteAttachmentFile", width: "500px"});
...
function deleteFileNameConfirm(file, type, uploadType, fileId, uploadFileFormParentId) {
	...
	else if (uploadType == "mailmaga" || uploadType == "event" || uploadType == "eventplace" || uploadType == "eventinstructor" || uploadType == "product" || uploadType == "eventcourse" || uploadType == "membership" || uploadType == "survey") {
		attachmentNum --;
		attachmentIconShow();
	}
...
function uploadAndSave(thisObj, uploadFileFormParentId) {
	...
	if(uploadFormCounter > 0) {
		uploadFormCounter = 1;
		$(uploadFileFormParentIdStr + " #fu-message").find("div[id^=new-upload-file]").first().before("<span id='new-file-flag'></span>");	
...
function uploadFile(basePath, uploadType, uploadObjectId, callBackFuncName, thisObjId, thisObjClkFun, thisObjText, uploadFormCounter, uploadFileFormParentId) {
	...
	if (uploadFileValue) {
		...
		if (uploadType == "originaldomain") {
		    ...
		    $("#file-upload-comment-form" + uploadFormCounter).remove();
			var uploadMaxCount = $('form[id^="file-upload-comment-form"]').length;
			uploadFormCounter ++ ;
