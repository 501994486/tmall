...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		setPager = function (pgid, tp){
			...
			$("#"+$.jgrid.jqID(pgid) )
			.append("<div id='"+pgcnt+"' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;height:100%;' role='row'><tbody><tr id='pager-target-one'><td id='"+lft+"' align='left'><select id='select-target-one'></select></td><td id='"+cent+"' align='center' style='white-space:pre;'></td><td id='"+rgt+"' align='right'></td></tr><tr id='pager-target-all'><td colspan='3' style='vertical-align: middle'><select id='select-target-all'></select></td></tr></tbody></table></div>")
			.attr("dir","ltr"); //explicit setting
			
			for (var q = 0; q < ts.p.targetOne.length; q++) {
				$('#select-target-one').append('<option value="' + ts.p.targetOne[q].command + '">' + ts.p.targetOne[q].menu + '</option>');
			}
			
			for (var q = 0; q < ts.p.targetAll.length; q++) {
				$('#select-target-all').append('<option value="' + ts.p.targetAll[q].command + '">' + ts.p.targetAll[q].menu + '</option>');
			}
