	...
	$(function() {
		...
		var u = navigator.userAgent.toLowerCase(),
			...
			css_string = '.jstree ul, .jstree li { margin:0px; padding:0px; list-style-type:none; min-width:32px; /*min-height:36px;*/ line-height: 0%;} ' +
				...
				'.jstree > ul > li {margin-tom: 0px; margin-bottom: 0px; margin-left:0px;} ' +
		...
		_fn : {
			init	: function () {
					...
					if(typeof(Storage) !== "undefined") {
						...
						if (jsLsTreeWidth && parseInt(jsLsTreeWidth) > 100) {
							...
							if ($('#' + treeId, $('#left-panel')).length > 0) {
								$('#left-panel').css('min-width', jsLsTreeWidth);
				...
				if (this.get_settings().core.lang.resizeDisabled){
					...
					// YS tree splitter
					$("#" + treeId + "-wrap .jstree-splitter").bind("mousedown", function(e) {
						$(this).css({"cursor":"col-resize", "background-color": "#ccc"});
						$(this).width(4);
					})
					...
					var jsTreeLPW = $('#left-panel').css('min-width') === undefined ? 0 : parseInt($('#left-panel').css('min-width').replace('px', ''));
					
					var oldPageX = 0;
					$("#" + treeId + "-wrap .jstree-splitter").draggable({
						cursorAt: { left: 0 },
						axis: "x",
						start : function(e, ui) {
							oldPageX = e.pageX;
						...
						drag : function(e, ui) {
							var margin=12,
								...
								containerLPW = $('body').width(), //$('#right-panel').parent().innerWidth(),
								nowJstreeLPW = $('#left-panel').outerWidth() + (e.pageX - oldPageX);// + (e.pageX - $(this).offset().left);
							oldPageX = e.pageX;
							if (nowJstreeLPW < minJsTreeLPW) {
								$(this).css("background-color", "");
								$('#left-panel').css('width', minJsTreeLPW);
								$(this).css('left', '');
								return true;
							}
							else if (containerLPW - nowJstreeLPW - intervalLPW < minRightLPW) {
								$(this).css("background-color", "");
								jsTreeLPW = containerLPW - minRightLPW - intervalLPW; //(e.pageX-margin));
								$('#left-panel').css('width', jsTreeLPW);
								$('#left-panel').css('min-width', jsTreeLPW);
								$('#right-panel').css('max-width',minRightLPW);
								$('#right-panel').css('max-width',`calc(100vw - ${intervalLPW}px - ${jsTreeLPW}px)`);
								$(this).css('left', '');
								return true;
							}
							
							jsTreeLPW = nowJstreeLPW;
							$('#left-panel').css('min-width', jsTreeLPW);
							$('#left-panel').css('width', jsTreeLPW);
							//$('#right-panel').css('max-width',containerLPW-jsTreeLPW-intervalLPW);
						...
						stop : function(e, ui) {
							$(this).width(6);
							//var eX = e.pageX || ($(this).offset().left) , spw=$(this).width(), 
							var fw=$('#left-panel').outerWidth();
				...
				this.get_container().css("overflow", "auto")
									...
									.css("height", this.get_settings().core.height);
				...
				setTimeout(function() {
					...
					if (p.core.lang.showBottomMenu && liObj.attr("nomenu") != '1' &&
						...
						if(!$("#" + treeId + "-wrap #js-tree-menu").is(":visible")) {
							$("#" + treeId + "-wrap #js-tree-menu").addClass("jstree-menu-icon-normal").css({"top":liObj.position().top, "height": liObj.children('a').height()}).attr({'nodeId':liObj.attr('id'), 'treeId':treeId}).show();
...
function refreshJsTree(treeDivId, url) {
	...
	// refresh wait icon
	$("#" + treeDivId).find("ul:eq(0)").append("<li class='jstree-last jstree-leaf'><ins style='background: #FFFFFF;'>&#160;</ins><a class='jstree-loading' href='javascript:void(0)'><ins class='jstree-icon'>&#160;</ins></a></li>");
...
function refreshJsTreeAndRestoreScroll(treeDivId, url) {
	...
	var $div = $("#" + treeDivId).parent().parent(),
