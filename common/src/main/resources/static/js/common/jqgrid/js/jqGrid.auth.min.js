...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		//YS START
		$('body').append('<div id="' + p.gridId + 'FullTextSearchDiv" style="z-index: 3000; width: 270px; height: 30px; background: white; position: absolute; display: none;"><div id="jqgrid-shadowbox-left"><div id="jqgrid-shadowbox-arrow-up"/><table style="margin-left: 2px; margin-top: 2px;"><tr><td><input type="hidden" id="jqGridSearchName"><input type="text" id="' + p.gridId + 'ftkeyword" name="' + p.gridId + 'ftkeyword" style="width: 204px; border: 1px solid #91c6db; outline: none;"></td><td><div class="btn-g"><a href="javascript:void(0)" id="jqGridSearchBtn"><span>'+p.searchBtnText+'</span></a></div></td></tr></table></div></div>');
		$('#jqGridSearchBtn').click(function(event) {
			$("#"+p.gridId+"FullTextSearchDiv").hide();
			if($.isFunction(window[$("#jqGridSearchName").val()])) {
				window[$("#jqGridSearchName").val()]($("#"+p.gridId+"ftkeyword").val());				
				$("#"+p.gridId+"ftkeyword").val("");
				$("#"+p.gridId+"FullTextSearchDiv").focus();
			}
		});
		
		var jqMenuTitle = '';
		for (var mi = 0; mi < p.targetOne.length; mi++) {
			if (p.targetOne[mi].command) {
				if ($.isFunction(window[p.targetOne[mi].menu])) {
					jqMenuTitle = window[p.targetOne[mi].menu]();
				}
				else {
					jqMenuTitle = p.targetOne[mi].menu;
				}
				p.targetOneMenu += "<li><a href='#' order='"+mi+"'>" + jqMenuTitle + "</a></li>";
			}
		}

		if (p.targetOneMenu && $('#' + p.gridId + 'MenuDetail').length == 0) {
			$('body').append("<ul id='" + p.gridId + "MenuDetail' class='jqgrid-contextmenu'>" + p.targetOneMenu + "</ul>");
			var jqgridMenuClickCount = 0;
			$(document).on('click', '#' + p.gridId + 'Menu', function(e){
				if ($('#' + $(this).attr('trid')).attr('aria-selected') == 'true') {
					var ptr = $(e.target).closest("tr.jqgrow");
					if (ptr.offset()) {
						$("#"+p.gridId+"MenuDetail").css("width", $("#"+p.gridId+"TargetOne").width() + 20);
						$("#"+p.gridId+"MenuDetail").css({"margin-left": ptr.offset().left - ($("#"+p.gridId+"MenuDetail").width() / 2) + 18, "margin-top": ptr.offset().top + ptr.height()/2 + 16});
					}
					if ($("#"+p.gridId+"MenuDetail").is(":visible")) {
						$("#"+p.gridId+"MenuDetail").hide();
					}
					else {
						$("#"+p.gridId+"MenuDetail").show();	
					}
					jqgridMenuClickCount = 0;
				}
				else {
					if (jqgridMenuClickCount < 2) {
						$('#'+p.gridId).jqGrid("setSelection", $(this).parents("div[trid]").attr('trid'));
						jqgridMenuClickCount++;
						$(this).click();
					}
				}
				return false;
			})
			.on("mouseenter", '.jqgrid-menu-icon', function(e) {
				$(this).css("background-color", "#fff");
			})
			.on("mouseleave", '.jqgrid-menu-icon', function(e) {
				if (!$("#"+p.gridId+"MenuDetail").is(":visible")) {
					$(this).css("background-color", "#F2F2F2");
				}
			});	

			$('#'+p.gridId+'MenuDetail a').click(function(e) {
				/*
				if(ts.p.multiselect) {
					$(ts).jqGrid("resetSelection");
				}
				*/
				$(ts).jqGrid("setSelection", $(this).parents("div[trid]").attr('trid'), true, e);
				if ($.isFunction(window[p.targetOne[$(this).attr("order")].command])) {
					window[p.targetOne[$(this).attr("order")].command]();
				}
				$("#" + p.gridId + "MenuDetail").hide();
				return false;
			});
		...
		$(document).bind("click", function(e) {
			$("#"+p.gridId+"MenuDetail").hide();
			$('.jqgrid-menu-icon').css("background-color", "#F2F2F2");
		});
		...
		addMulti = function(rowid,pos,irow){
			var	v = "<input role=\"checkbox\" type=\"checkbox\""+" id=\"jqg_"+ts.p.id+"_"+rowid+"\" class=\"cbox\" name=\"jqg_"+ts.p.id+"_"+rowid+"\"/>"
				/*+ "<label for='jqg_"+ts.p.id+"_"+rowid+"' style='margin-left: 20px;'>&nbsp;</label>"*/,
			prp = formatCol( pos,irow,'',null, rowid, true);
			//YS
			if (ts.p.showRowNumbers === false) {
				v = '';
			}
			return "<td role=\"gridcell\" "+prp+">"+v+"</td>";
		},
		...
		updatepager = function(rn, dnd) {
			...
			if(rn===true && ts.p.rownumbers === true && ts.p.showRowNumbers === true) {
		...
		setPager = function (pgid, tp){
			...
			//YS
			var targetMenus = "<div id='"+pgcnt+"' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;height:100%;' role='row'><tbody>";
			targetMenus += "<tr><td id='"+lft+"' align='left' style='border-bottom : 1px solid #CCCCCC;'>";
			if (ts.p.targetOne.length > 0) {
				targetMenus += "<select id='" + ts.p.gridId + "TargetOne' style='margin-left: 1px; color: #666;' onchange='targetMenuClick(this);'></select>";
			}
			targetMenus += "</td><td id='"+cent+"' align='center' style='white-space:pre;border-bottom : 1px solid #CCCCCC;'></td><td id='"+rgt+"' align='right' style='border-bottom : 1px solid #CCCCCC;'></td></tr>";
			
			if (ts.p.targetAll.length > 0) {
					targetMenus += "<tr id='pager-target-all' style='height: 25px'><td colspan='3' class='pager-target-all-td' style='vertical-align: middle'><select id='" + ts.p.gridId + "TargetAll' style='margin-left: 1px; margin-top: 1px; color: #666;' onchange='targetMenuClick(this);'></select></td></tr>"; 
					targetMenus += "</tbody></table></div>";
				$("#"+$.jgrid.jqID(pgid)).append(targetMenus).attr("dir","ltr"); //explicit setting
			}
			else {
				targetMenus += "</tbody></table></div>";
				$("#"+$.jgrid.jqID(pgid))
			...
			if (ts.p.targetOne.length > 0) {
				for (var q = 0; q < ts.p.targetOne.length; q++) {
					$('#' + ts.p.gridId + 'TargetOne').append('<option value="' + ts.p.targetOne[q].command + '">' + ts.p.targetOne[q].menu + '</option>');
				}
			}
			
			for (var q = 0; q < ts.p.targetAll.length; q++) {
				$('#' + ts.p.gridId + 'TargetAll').append('<option value="' + ts.p.targetAll[q].command + '">' + ts.p.targetAll[q].menu + '</option>');
			}
			
			if(ts.p.rowList.length >0 && ts.p.pagerpos != "hidden"){
				str = "<td>" + ts.p.pg2text + "</td>";
				str += "<td dir='"+dir+"'>";
			...
			if(ts.p.pgbuttons===true) {
				...
				pgl += "<td id='"+po[0]+"' class='ui-pg-button ui-corner-all'><span class='fa fa-step-backward'></span></td>";
				pgl += "<td id='"+po[1]+"' class='ui-pg-button ui-corner-all'><span class='fa fa-play fa-flip-horizontal'></span></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				pgl += "<td id='"+po[2]+"' class='ui-pg-button ui-corner-all'><span class='fa fa-play'></span></td>";
				pgl += "<td id='"+po[3]+"' class='ui-pg-button ui-corner-all'><span class='fa fa-step-forward'></span></td>";
			} else if (pginp !== "") { pgl += pginp; }
			if(dir=="ltr") { pgl += str; }
			pgl += "</tr></tbody></table>";
			if(ts.p.viewrecords===true) {$("td#"+pgid+"_"+ts.p.recordpos,"#"+pgcnt).append("<div dir='"+dir+"' style='text-align:"+ts.p.recordpos+"' class='ui-paging-info'></div>");}
			$("td#"+pgid+"_"+ts.p.pagerpos,"#"+pgcnt).append(pgl);
			tdw = $(".ui-jqgrid").css("font-size") || "12px";
		...
		sortData = function (index, idxcol,reload,sor){
			...
			if(!ts.p.viewsortcols[0]) {
				if(ts.p.lastsort != idxcol) {
					...
					//YS
					$("span.s-ico",previousSelectedTh).parent().children("a").css("font-weight", "normal");
					$("span.s-ico",newSelectedTh).parent().children("a").css("font-weight", "bold");
				}
			}
		...
		if(this.p.rownumbers) {
			...
			this.p.colModel.unshift({name:'rn',width:ts.p.rownumWidth,sortable:false,resizable:false,hidedlg:true,search:false,align:'right',fixed:true});
		}
		...
		for(i=0;i<this.p.colNames.length;i++){
			...
			// YS
			if (ts.p.colModel[i].name == ts.p.sortname) {
				fontStyle = "style='font-weight: bold;'";
			}
			
			if (ts.p.colModel[i].sortable === false) {
				thead += "<div style='cursor: default;' id='jqgh_"+ts.p.id+"_"+ts.p.colModel[i].name+"' "+tdc+">"+ts.p.colNames[i];	
			}
			else {
				thead += "<div id='jqgh_"+ts.p.id+"_"+ts.p.colModel[i].name+"' "+tdc+">"+"<a href=# class='sortable' " + fontStyle + ">"+ts.p.colNames[i]+"</a>&nbsp;";
			}
		...
		$("th",thr).each(function ( j ) {
			...
			if (ts.p.colModel[j].searchable) {
				$(">div",this).append($("<a href='#' class='ui-jqgrid-searchable'>search</a>").click(function(event) {
					showJqGridSearchForm(event.currentTarget, new String(ts.p.gridId), new String(ts.p.colModel[j].name), ts.p.colModel[j].searchForm);
				}));
			}
		...
		$(".sortable", thr).click(function(e) {
			...
			var t = $(e.target).closest(s);
			if (t.length != 1) { return; }
			var ci = getColumnHeaderIndex($(this).parent());
			if (!ts.p.viewsortcols[2]) { r=true;d=t.attr("sort"); }
			sortData($(this).parent().attr('id'), ci, r, d);
			return false;
		});
		...
		$(ts).before(grid.hDiv).click(function(e) {
			...
			setTimeout("$('#" +  ts.p.gridId + "Menu').hide();", 0);	
			return false;
		})
		.dblclick(function(e) {
			td = e.target;
		...
		$(grid.bDiv)
			.attr('id', ts.p.gridId + "TableBox")
			.append($('<div style="position:relative;'+(isMSIE && $.browser.version < 8 ? "height:0.01%;" : "")+'"></div>').append('<div></div>').append(this))
			.addClass("ui-jqgrid-bdiv")
			.css({overflow: 'visible', height: ts.p.height+(isNaN(ts.p.height)?"":"px"), width: (grid.width)+"px"})
		...
		var arf = ts.p.hidegrid===true ? $("<a role='link' href='javascript:void(0)'/>").addClass('ui-jqgrid-titlebar-close HeaderButton')
		.append("<span class='ui-icon ui-icon-circle-triangle-n'></span>").css((dir=="rtl"?"left":"right"),"0px") : "";
			
		var fts = ts.p.fulltextSearch===true ? $("<a href='#' onclick=\"searchIconClick(this, '" + ts.p.gridId + "');\"/>").addClass('ui-jqgrid-titlebar-fulltextsearch')
			.append("<span class='ui-icon ui-icon-search'></span>") : "";
			
		if (fts != "") {
			if (arf != "") {
				fts.css((dir=="rtl"?"left":"right"),"20px");
			}
			else {
				fts.css((dir=="rtl"?"left":"right"),"0px");
			}
		}
			
		$(grid.cDiv).append(arf).append(fts).append("<span class='ui-jqgrid-title"+(dir=="rtl" ? "-rtl" :"" )+"'>"+ts.p.caption+"</span>")
...
$.jgrid.extend({
	...
	setSelection : function(selection,onsr, e) {
		return this.each(function(){
			...
			//YS START
			//if (e && e.target && !$t.p.multiselect) {
			if (e && e.target) {
				var ptr = $(e.target).closest("tr.jqgrow"), trRowNum = ptr.children("td:first"), pGridMenu = $('#' +  $t.p.gridId + 'Menu');
				if (pGridMenu.length > 0 && ptr[0].id != pGridMenu.attr('trid')) {
					pGridMenu.remove();
				}
				if (trRowNum && $('#' +  $t.p.gridId + 'Menu').length == 0 && $t.p.targetOne.length > 0) {
					trRowNum.prepend(getTargetOneMenuStr($t.p));
					$('#' + $t.p.gridId + 'Menu').attr('trid', ptr[0].id);
				}
			}
			//YS END
		});
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					if(ni){
						...
						//YS
						if (t.p.showRowNumbers) {
							row[row.length] = "<td role=\"gridcell\" class=\"ui-state-default jqgrid-rownum\" "+prp+">0</td>";	
						}
						else {
							row[row.length] = "<td role=\"gridcell\" class=\"ui-state-default jqgrid-rownum\" "+prp+"></td>";
						}
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			...
			$("#gbox_"+$.jgrid.jqID($t.p.id)).css("position","relative").css("width","");
			$("#gview_"+$.jgrid.jqID($t.p.id)).css("position","relative").css("width","");
			$($t.grid.bDiv).css("position","relative").css("width","");
			$($t.grid.hDiv).css("position","relative").css("width","");
...
function jqGridGetPos(el) {
    for (var lx=0, ly=0; el != null; lx += el.offsetLeft, ly += el.offsetTop, el = el.offsetParent);
    return {"left": lx - 256, "top": ly + 40};
}

function showJqGridSearchForm(thisObj, gridId, colName, colModelSearchForm) {
	if (colModelSearchForm && $.isFunction(colModelSearchForm)) {
		colModelSearchForm();
	}
	else {
		if ($("#" + gridId + "FullTextSearchDiv").is(":visible") && $("#jqGridSearchName").val() && $("#jqGridSearchName").val().indexOf(colName.capitalize()) != -1) {
			$("#" + gridId + "FullTextSearchDiv").hide();
		}
		else {
			$("#" + gridId + "ftkeyword").val('');
			$("#" + gridId + "FullTextSearchDiv").css(jqGridGetPos(thisObj)).show();
			$("#" + gridId + "ftkeyword").focus();
			$('#jqGridSearchName').val(gridId + colName.capitalize() + "Search");
		}
	}
}

function searchIconClick(thisObj, gridId) {
	if ($("#" + gridId + "FullTextSearchDiv").is(":visible") 
			&& $("#jqGridSearchName").val() 
			&& $("#jqGridSearchName").val().indexOf('FulltextSearch') != -1) {
		$("#" + gridId + "FullTextSearchDiv").hide();	
	}
	else {
		$("#" + gridId + "FullTextSearchDiv").css(jqGridGetPos(thisObj)).show();
		$("#jqGridSearchName").val(gridId + "FulltextSearch");
		$("#" + gridId + "ftkeyword").focus();
	}
	return false;
}

function targetMenuClick(obj) {
	if ($.isFunction(window[obj.value])) {
		window[obj.value]();
		obj.options[0].selected = true;
	}
}

function getTargetOneMenuStr(p) {
	return "<div id='" + p.gridId + "Menu' style='display: inline-block; float: left; text-align: left;'>" +
			"<div class='jqgrid-menu-icon' id='" + p.gridId + "_TargetOneMenu'>" + p.operationBtnText + "</div></div>";
}
