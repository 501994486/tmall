...
$.fn.autoExpand = function(options) {
	...
	autoExpand.prototype.autoExpand = function() {
		...
		if (totalHeight >= maxHeight) {
			...
			//scrollHeight = maxHeight;del.ylq

			// if we can't expand, ensure overflow-y is set to auto
			//this.$element.css('overflow-y', 'auto');del.ylq
			this.$element.css('overflow-y', 'hidden'); //add.ylq
		} else {
		...
		if (scrollHeight !== this.$element.height() + padding &&
			...
			//this.$element.animate({ height: scrollHeight }, 0);
			this.$element.height(scrollHeight);
			//alter the position of errorMsg after autoExpand;
			if ($('div[class$="formError"]')) {
				$('div[class$="formError"]').each(function() {
					var currentTextareaTop = $(this).next().position().top;
					var left = $(this).position().left;
					$(this).css({"top":currentTextareaTop,"left":left});
				}); 
			}
			
			if (this.initFlag == false) {
				//$(document).scrollTop(oldScrollTop + scrollHeight - height);
				if ($(this.$element).parents('.ysdialog-contents').length > 0) {
					$(this.$element).closest('.ysdialog-contents').scrollTop(scrollHeight);
				}
			}
		}
		
		$(document.getElementsByName("textarea-comment")).scrollTop(scrollHeight);
