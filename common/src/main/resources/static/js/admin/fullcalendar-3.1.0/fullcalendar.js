...
var HitDragListener = DragListener.extend({
	...
	handleInteractionStart: function(ev) {
		...
		if (ev) {
			...
			if (subjectEl) {
				subjectRect = getOuterRect(subjectEl); // used for centered as well
...
Grid.mixin({
	...
	handleSegMouseover: function(seg, ev) {
		...
		if(event._id.indexOf('|')>-1){
			event._id=event._id.replace('|','or');
		}
		
		if(event._id.indexOf('_rep')>-1){
			$('.fc-event-'+event._id).css('background-color','#ffe1d4');
		}
		else{
			$('.fc-event-'+event._id).css('background-color','#d5f2ff');
		}
		
		$('.fc-event-'+event._id).addClass('change');
		$('.fc-event-'+event._id).addClass('che');
		$('.fc-event-'+event._id+'.fc-not-start  .fc-title').css('color','var(--base-txt-color)');
	...
	handleSegMouseout: function(seg, ev) {
		$('.fc-event').css('background-color','#EEF5FA');
		$('.fc-event').removeClass('change');
		$('.fc-event').removeClass('che');
		//$('.fc-event').removeClass('cha');
		$('.fc-not-start .fc-title').css('color','var(--base-txt-color)');
	...
	getSegClasses: function(seg, isDraggable, isResizable) {
		...
		if(classes.indexOf('fc-start')>-1&&classes.indexOf('fc-end')<=-1){
			classes.push('fc-start-only');
			//classes.push('fccnm');
		}
		if(classes.indexOf('fc-end')>-1&&classes.indexOf('fc-start')<=-1){
			classes.push('fc-end-only');
		}
		if((classes.indexOf('fc-start')>-1)&&(classes.indexOf('fc-end')>-1)){
			if(event.end!=null){
				classes.push('fc-event-mulday');
			}
		}
		if(event._id.indexOf('|')>-1){
			event._id=event._id.replace('|','or');
		}
		classes.push('fc-event-'+event._id);
		if (isDraggable) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
						...
						if (buttonClick) {
							...
							if(innerHtml.indexOf('fc-icon-circle-triangle-w')>-1){
								innerHtml = "<span class='fc-text-arrow'>◄</span>";
							}
							if(innerHtml.indexOf('fc-icon-circle-triangle-e')>-1){
								innerHtml = "<span class='fc-text-arrow'>►</span>";
							}
							if(innerHtml.indexOf('today')>-1){
								innerHtml = "今日";
							}
...
Calendar.defaults = {
	...
	monthYearFormat: 'YYYY年M月', // required for en. other locales rely on datepicker computable option
...
//		week: 'week',
//		day: 'day'
		prev: "<span class='fc-text-arrow'>&laquo;</span>",
		next: "<span class='fc-text-arrow'>&raquo;</span>",
