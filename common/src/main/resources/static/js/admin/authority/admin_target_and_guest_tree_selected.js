export default function tree($) {
	...
	var cb_div_left = "<div id='treename-leafId-div' class='tree-select-icon-box'><i class='fa fa-circle tree-select-icon help-bubble-popup' guidance='対象にする'></i> ";
	var cb_info = "<input type='checkbox' class='function-cb membership-cb' onclick='changeEvent(this);' id='treename-leafId-cb' name='treename-leafId-cb-name' value='cbval'/>"
				+ '<i class="fa fa-check-circle tree-node-selected-icon help-bubble-popup" guidance="対象から除外する" aria-hidden="true" style="display: none;"></i>'
				+ '<i class="fa fa-times-circle tree-node-excepted-icon help-bubble-popup" guidance="対象にする" aria-hidden="true" style="display: none;"></i>';
	var cb_div_right = '<i id="treename-leafId-cb-img" class="fa fa-thumb-tack oneself-stand-alone-mark" aria-hidden="true" style="display: none;"></i>'
		//+'<i id="treename-leafId-cb-stand-alone-help" style="overflow:right;margin-left:45px;margin-top:-15px; display:none;" class="fa fa-question-circle fa-question-circle-green question-bubble-popup" position="right" custom-width="200" guidance="<m>この赤ピンは限定の対象から除外するために直接指定した箇所で、下のグループも対象から除外されチェックマークが無くなります</m>"></i><i id="treename-leafId-cb-oneself-stand-alone-help" style="overflow:right;margin-left:45px;margin-top:-15px; display:none;" class="fa fa-question-circle fa-question-circle-green question-bubble-popup" position="right" custom-width="200" guidance="<m>この赤ピンは限定の対象として直接指定した箇所で、下のグループも対象となってチェックマークが付きます</m>">'
		+'</i></div>';
	...
	$.fn.jstreecheckbox = function(settings) {
				...
				if(showTopLine == true){
					//if ($(this).attr("rel") == "default" || $(this).attr("rel") == "folder") {
						...
						for ( var i = 0; i < cbSize; i++) {
							...
							$(this).children(".tree-select-icon-box").unbind("click").bind("click",function(){
								clickNode(leafId);
							});
							$(this).children("a").find('div[id^="count-pin-"]')
								.addClass("help-bubble-popup")
								.attr("guidance","対象にする")
		...
		var cb_div_left = "<div id='treename-leafId-div' class='jqgrid-select-icon-box'><i class='fa fa-circle jqgrid-select-icon help-bubble-popup' guidance='対象にする'></i> ";
		var cb_info = "<input type='checkbox' class='function-cb membership-cb' onclick='changeEvent(this);' id='treename-leafId-cb' name='treename-leafId-cb-name' value='cbval'/>"
					+ '<i class="fa fa-check-circle tree-node-selected-icon help-bubble-popup" guidance="対象から除外する" aria-hidden="true" style="display: none;"></i>'
					+ '<i class="fa fa-times-circle tree-node-excepted-icon help-bubble-popup" guidance="対象にする" aria-hidden="true" style="display: none;"></i>';
			...
			for ( var j = 0; j < ids.length; j++) {
				...
				for ( var i = 0; i < cbSize; i++) {
					...
					result = result.replace(/changeEvent/g, clickEvent);
					// 树节点添加对应的权限复选框
					$("#" + ids[j]).children("td").eq(2).append(result);
					// $("#" + ids[j]).append(result);
				}
			}
		};
