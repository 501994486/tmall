...
function ysLoadedJstree(treeName, url, data, authSetting) {
	...
	addStyleClass(treeName);
}

function addStyleClass (treeName){
	setTimeout( function(){
		$("#" + treeName + "-tree li").each(function(j) {
			var relVal = $(this).attr("rel");
			if (typeof(relVal) != "undefined" && relVal != "folder2" && relVal != "folder3") {
				var resId = $(this).attr("id");
				$countPin = $('#count-pin-' + resId);
				var display =$countPin.css('display');
				if(display != 'none'){
					$("#" + resId).addClass("subchild-selected");
				}
			}
		});
	},400);
...
function iconDisplay(treeName, leafId, methodName){
	$('#'+leafId).children('a').parents('li').each(function() {
		var id = $(this).attr("id");
	...
	if (methodName == "addgrid") {
		$('#'+ leafId + '>ul>li').each(function() {
			if ($(this).hasClass("subchild-selected")){
				var objId = $(this).attr("id");
				setTimeout( function(){
					$("#" + objId).children("a").removeClass("jstree-clicked");
				},200);
			}
		});
	}else if (methodName == "removegrid") {
		setTimeout( function(){
			var $countPin = $('#count-pin-' + leafId);
			var display =$countPin.css('display');
			if(display != 'none'){
				$("#" + leafId).children("a").removeClass("jstree-clicked");
			}
		},200);
	}
}
...
function addgrid(treeName, url, leafId, treeBox, authSetting) {
	iconDisplay(treeName, leafId, "addgrid");
...
//	}
	...
	if (treeName == 'auth-membership') {
		var tbody_td_last_width = $("#" + treeName + "-grid-" + leafId).children("tbody").children("tr").children("td:last").width();
		$("#" + treeName + "-grid-" + leafId).css("width", "97%");
		$("#" + treeName + "-grid-" + leafId).children("tbody").children("tr").children("td:last").css("width", tbody_td_last_width - 20);
	}
}
...
function removegrid(treeName, leafId) {
	iconDisplay(treeName, leafId, "removegrid");
		...
		success : function(data) {
			if (data.result == _STATUS_SUCCESS) {
								...
								if (authValue.length == 4) {
									...
									if (objGroupIds && (objGroupIds instanceof Array)) {
										for ( var j = 0; j < objGroupIds.length; j++) {
											$('#count-pin-' + objGroupIds[j]).html('<i class="jstree-icon icon-arrow subchild-stand-alone-mark" aria-hidden="true"></i>').attr('style', "display: inline-block;");
										}
									}
								}
							}
				...
				for ( var i = 0; i < data.lightPinset.length; i++) {
					...
					$('#count-pin-' + resId).html('<i class="jstree-icon icon-arrow subchild-stand-alone-mark" aria-hidden="true"></i>').attr('style', "display: inline-block;");
				}
				...
				for ( var i = 0; i < data.functionLightPinset.length; i++) {
					...
					$('#count-pin-' + objId).html('<i class="jstree-icon icon-arrow subchild-stand-alone-mark" aria-hidden="true"></i>').attr('style', "display: inline-block;");
				}
...
var disrid;
var resourceIds = new Array();
function authMemberListFresh() {
	if (parent.$('#instructorDetailFlag').prop('checked')) {
		userArray.push(parent.$('#entryUserId').val());
	}
	...
	for (var i = 0; i < userArray.length; i++){
		if (userArray[i] != ""){
			resourceId.push(userArray[i]);
		}
	}
	resourceId = resourceId.distinct();
	
	memberIdArray = memberIdArray.distinct();
	for(var i in memberIdArray) {
		memberIdArray[i] = "me" + memberIdArray[i] + '#' + _AUTH_YES
	}
	
	for(var i in resourceId) {
		resourceId[i] = "us" + resourceId[i] + '#' + _AUTH_YES
	}
	
	var newResourceIds = new Array();
	newResourceIds = memberIdArray.concat(resourceId);
	
	var authDelArray = new Array();
	for (var i in memberIdDelArray){
		authDelArray[i] = "me" + memberIdDelArray[i] +  "#" + _AUTH_YES;
	}
	memberIdDelArray = [];
	
	var domainUrl = top.jzAppVars().domainUrl;
	$.post(domainUrl + "/admin/common/target/get-member-auth-num/" + 0 + "/" + objectAuthType, 
			{authList: newResourceIds.toString(), authDelList: authDelArray.toString()}, function(data) {
				var userIdList = data.userIdList;
				var userIdDelList = data.userIdDelList;
				for (var i in userIdDelList) {
					var userTabIndex = userIdList.indexOf(userIdDelList[i]);
					if (userTabIndex > -1) {
						userIdList.splice(userTabIndex, 1);
					}
				}
				disrid = userIdList.distinct(); 
				userArray = disrid;
				userAddArray = disrid;
				resourceIds = disrid;
				alert(userIdList)
				updateAuthTrees(0, userIdList.distinct());
	}, "json");
}

function updateAuthTrees(objectId, newResourceIdArray){
	for (var i in newResourceIdArray) {
		newResourceIdArray[i] = "us" + newResourceIdArray[i] + "#" + _AUTH_YES;
	}
	...
	$.ajax({
		...
		data : {
			objectId : objectId,
			objectAuthType : objectAuthType,
			authList : newResourceIdArray.toString()
		},
		type : 'POST',
		async : false,
		success : function(data) {
					...
					for ( var i = 0; i < data.resourceAuthList.length; i++) {
						...
						if (authValue instanceof Array) {
							...
							if (authValue[2] == _AUTH_INHERITED) {
								...
								//$("input[value=" + authValue[0] + "]").parent().children("i[id$='cb-img']").show();
								$("input[value=" + authValue[0] + "]").addClass("stand-alone");
								//$("#" + authValue[0]).find("i[id$='cb-stand-alone-help']").show();
